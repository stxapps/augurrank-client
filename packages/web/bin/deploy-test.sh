...
aws s3 sync out/_next s3://test-augurrank-web/_next --size-only --delete --cache-control max-age=31536000 --profile justnote

aws s3 sync out/nextImageExportOptimizer s3://test-augurrank-web/nextImageExportOptimizer --size-only --delete --cache-control max-age=31536000 --profile justnote
